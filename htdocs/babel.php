	...
	case 'user_home':
		if ($_SESSION['babel_ua']['DEVICE_LEVEL'] < 3 && $_SESSION['babel_ua']['DEVICE_LEVEL'] > 0) {
				...
				if ($do == 'me') {
					...
					if (isset($_GET['user_nick'])) {
						...
						if (mb_strlen($user_nick_real, 'UTF-8') > 0) {
							$sql = "SELECT usr_id, usr_geo, usr_nick, usr_brief, usr_religion, usr_religion_permission, usr_gender, usr_portrait, usr_hits, usr_logins, usr_created, usr_lastlogin, usr_lastlogin_ua FROM babel_user WHERE usr_nick = '{$user_nick_real}'";
			...
			if ($options['mode'] == 'random') {
				$sql = "SELECT usr_id, usr_geo, usr_nick, usr_brief, usr_religion, usr_religion_permission, usr_gender, usr_portrait, usr_hits, usr_logins, usr_created, usr_lastlogin, usr_lastlogin_ua FROM babel_user ORDER BY rand() LIMIT 1";
				$rs = mysql_query($sql, $p->db);
	...
	case 'user_update':
		if ($p->User->vxIsLogin()) {
			if (strtoupper($_SERVER['REQUEST_METHOD']) != 'POST') {
				...
				if ($rt['errors'] == 0) {
					$p->Validator->vxUserUpdateUpdate($rt['usr_full_value'], $rt['usr_nick_value'], $rt['usr_email_notify_value'], $rt['usr_brief_value'], $rt['usr_gender_value'], $rt['usr_religion_value'], $rt['usr_religion_permission_value'], $rt['usr_addr_value'], $rt['usr_telephone_value'], $rt['usr_identity_value'], $rt['usr_width_value'], $rt['usr_sw_shuffle_cloud_value'], 
