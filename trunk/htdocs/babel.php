	...
	case 'user_home':
		if ($_SESSION['babel_ua']['DEVICE_LEVEL'] < 3 && $_SESSION['babel_ua']['DEVICE_LEVEL'] > 0) {
				...
				if ($do == 'me') {
					...
					if (isset($_GET['user_nick'])) {
						...
						if (mb_strlen($user_nick_real, 'UTF-8') > 0) {
							$sql = "SELECT usr_id, usr_geo, usr_nick, usr_brief, usr_gender, usr_portrait, usr_hits, usr_logins, usr_created, usr_lastlogin, usr_lastlogin_ua FROM babel_user WHERE usr_nick = '{$user_nick_real}'";
			...
			if ($options['mode'] == 'random') {
				$sql = "SELECT usr_id, usr_geo, usr_nick, usr_brief, usr_gender, usr_portrait, usr_hits, usr_logins, usr_created, usr_lastlogin, usr_lastlogin_ua FROM babel_user ORDER BY rand() LIMIT 1";
				$rs = mysql_query($sql, $p->db);
