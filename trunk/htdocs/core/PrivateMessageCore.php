...
*  Subversion Keywords:
*
*  $Id: PrivateMessageCore.php 60 2007-02-05 08:00:48Z livid $
*  $LastChangedDate: 2007-02-05 16:00:48 +0800 (Mon, 05 Feb 2007) $
*  $LastChangedRevision: 60 $
*  $LastChangedBy: livid $
*  $URL: http://svn.cn.v2ex.com/svn/babel/trunk/htdocs/core/PrivateMessageCore.php $
*/
...
class PrivateMessage {
	...
	public function vxInbox() {
		if ($this->User->vxIsLogin()) {
			...
			$p['ext'] = '.vx';
			$sql = "SELECT COUNT(msg_id) FROM babel_message WHERE msg_rid = {$this->User->usr_id} AND msg_draft = 0 AND msg_rdeleted = 0";
			$rs = mysql_query($sql, $this->db);
			...
			if ($p['items'] > 0) {
				...
				$sql = "SELECT msg_id, msg_body, msg_created, msg_opened, usr_nick FROM babel_message, babel_user WHERE msg_rid = {$this->User->usr_id} AND msg_draft = 0 AND msg_rdeleted = 0 AND msg_sid = usr_id ORDER BY msg_sent DESC LIMIT " . $p['sql'] . "," . BABEL_MSG_PAGE;
				$rs = mysql_query($sql, $this->db);
				echo('<div id="main">');
				echo('<div id="tool">');
				if ($p['total'] > 1) {
					$this->vxDrawPages($p, ' received');
				} else {
	...
	private function vxDrawPages($p, $suffix) {
		if ($p['start'] != 1) {
		...
		echo('<strong class="p_info">' . $p['items'] . $suffix . '</strong>');
	}
